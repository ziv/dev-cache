...
export default async function wrap<T = unknown>(
  ...
  const SOCKET_PATH = process.env.DEVCACHE_SOCKET ??
    `${process.env.TMPDIR}/dev-cache.sock`;
  // is socket available?
  ...
  // is server available?
  const statusReply = await request("status").catch(
    () => ({ status: "error" } as Reply),
  );
